...
export async function toolToAPISchema(tool: Tool, options: {
    ...
    if (!base) {
        ...
        let input_schema = ('inputJSONSchema' in tool && tool.inputJSONSchema
            ...
            : zodToJsonSchema(tool.inputSchema)) as Anthropic.Tool.InputSchema;
 ...
 *    - Everything else concatenated (cacheScope='org')
 */
export function splitSysPromptPrefix(systemPrompt: SystemPrompt, options?: {
...
export function appendSystemContext(systemPrompt: SystemPrompt, context: {
    [k: string]: string;
}): string[] {
    return [
        ...systemPrompt,
...
export function prependUserContext(messages: Message[], context: {
    [k: string]: string;
}): Message[] {
 ...
 */
export async function logContextMetrics(mcpConfigs: Record<string, ScopedMcpServerConfig>, toolPermissionContext: ToolPermissionContext): Promise<void> {
...
export function normalizeToolInput<T extends Tool>(tool: T, input: z.infer<T['inputSchema']>, agentId?: AgentId): z.infer<T['inputSchema']> {
    switch (tool.name) {
...
export function normalizeToolInputForAPI<T extends Tool>(tool: T, input: z.infer<T['inputSchema']>): z.infer<T['inputSchema']> {
    switch (tool.name) {
        case EXIT_PLAN_MODE_V2_TOOL_NAME: {
