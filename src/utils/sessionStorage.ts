...
import { type ContentReplacementEntry, type ContextCollapseCommitEntry, type ContextCollapseSnapshotEntry, type Entry, type FileHistorySnapshotMessage, type LogOption, type PersistedWorktreeSession, type SerializedMessage, sortLogs, type TranscriptMessage, } from '../types/logs.js';
import type { AssistantMessage, AttachmentMessage, Message, SystemCompactBoundaryMessage, SystemMessage, UserMessage, } from '../types/message.js';
...
class Project {
    ...
    private writeQueues = new Map<string, Array<{
        entry: Entry;
        resolve: () => void;
    }>>();
    ...
    async insertMessageChain(messages: Transcript, isSidechain: boolean = false, agentId?: string, startingParentUuid?: UUID | null, teamInfo?: {
        teamName?: string;
        agentName?: string;
    }) {
        return this.trackWrite(async () => {
...
export async function recordTranscript(messages: Message[], teamInfo?: TeamInfo, startingParentUuidHint?: UUID, allMessages?: readonly Message[]): Promise<UUID | null> {
...
export async function recordSidechainTranscript(messages: Message[], agentId?: string, startingParentUuid?: UUID | null) {
...
export async function recordFileHistorySnapshot(messageId: UUID, snapshot: FileHistorySnapshot, isSnapshotUpdate: boolean) {
 ...
 */
function findLatestMessage<T extends {
    timestamp: string;
}>(messages: Iterable<T>, predicate: (m: T) => boolean): T | undefined {
 ...
 */
function recoverOrphanedParallelToolResults(messages: Map<UUID, TranscriptMessage>, chain: TranscriptMessage[], seen: Set<UUID>): TranscriptMessage[] {
    type ChainAssistant = Extract<TranscriptMessage, {
        type: 'assistant';
    }>;
 ...
 */
function buildFileHistorySnapshotChain(fileHistorySnapshots: Map<UUID, FileHistorySnapshotMessage>, conversation: TranscriptMessage[]): FileHistorySnapshot[] {
...
export async function loadTranscriptFromFile(filePath: string): Promise<LogOption> {
    if (filePath.endsWith('.jsonl')) {
        const { messages, summaries, customTitles, tags, fileHistorySnapshots, contextCollapseCommits, contextCollapseSnapshot, leafUuids, contentReplacements, worktreeStates, } = await loadTranscriptFile(filePath);
        if (messages.size === 0) {
...
function convertToLogOption(transcript: TranscriptMessage[], value: number = 0, summary?: string, customTitle?: string, fileHistorySnapshots?: FileHistorySnapshot[], tag?: string, fullPath?: string, agentSetting?: string, contentReplacements?: ContentReplacementRecord[]): LogOption {
...
export async function saveCustomTitle(sessionId: UUID, customTitle: string, fullPath?: string, source: 'user' | 'auto' = 'user') {
 ...
 */
export async function linkSessionToPR(sessionId: UUID, prNumber: number, prUrl: string, prRepository: string, fullPath?: string): Promise<void> {
    const resolvedPath = fullPath ?? getTranscriptPathForSession(sessionId);
    appendEntryToFile(resolvedPath, {
        type: 'pr-link',
        sessionId,
...
export async function saveAgentName(sessionId: UUID, agentName: string, fullPath?: string, source: 'user' | 'auto' = 'user') {
    const resolvedPath = fullPath ?? getTranscriptPathForSession(sessionId);
...
export async function loadFullLog(log: LogOption): Promise<LogOption> {
    ...
    try {
        const { messages, summaries, customTitles, tags, agentNames, agentColors, agentSettings, prNumbers, prUrls, prRepositories, modes, worktreeStates, fileHistorySnapshots, contentReplacements, contextCollapseCommits, contextCollapseSnapshot, leafUuids, } = await loadTranscriptFile(sessionFile);
        if (messages.size === 0) {
            return log;
        }
 ...
 */
export async function searchSessionsByCustomTitle(query: string, options?: {
    limit?: number;
    exact?: boolean;
}): Promise<LogOption[]> {
 ...
 */
export async function loadTranscriptFile(filePath: string, opts?: {
 ...
 */
const getSessionMessages = memoize(async (sessionId: UUID): Promise<Set<UUID>> => {
...
export async function getLastSessionLog(sessionId: UUID): Promise<LogOption | null> {
    ...
    const { messages, summaries, customTitles, tags, agentSettings, worktreeStates, fileHistorySnapshots, contentReplacements, contextCollapseCommits, contextCollapseSnapshot, } = await loadSessionFile(sessionId);
    if (messages.size === 0)
        return null;
 ...
 * @returns List of message logs sorted by date
 */
export async function loadAllProjectsMessageLogs(limit?: number, options?: {
    skipIndex?: boolean;
...
export async function loadAllProjectsMessageLogsProgressive(limit?: number, initialEnrichCount: number = INITIAL_ENRICH_COUNT): Promise<SessionLogResult> {
    const projectsDir = getProjectsDir();
    let dirents: Dirent[];
    try {
        dirents = await readdir(projectsDir, { withFileTypes: true });
...
export async function loadSameRepoMessageLogs(worktreePaths: string[], limit?: number, initialEnrichCount: number = INITIAL_ENRICH_COUNT): Promise<LogOption[]> {
    const result = await loadSameRepoMessageLogsProgressive(worktreePaths, limit, initialEnrichCount);
    return result.logs;
}
export async function loadSameRepoMessageLogsProgressive(worktreePaths: string[], limit?: number, initialEnrichCount: number = INITIAL_ENRICH_COUNT): Promise<SessionLogResult> {
 ...
 */
export async function loadSubagentTranscripts(agentIds: string[]): Promise<{
    [agentId: string]: Message[];
}> {
 ...
 */
export async function getSessionFilesWithMtime(projectDir: string): Promise<Map<string, {
    path: string;
    mtime: number;
    ctime: number;
    size: number;
}>> {
    const sessionFilesMap = new Map<string, {
        path: string;
        mtime: number;
        ctime: number;
        size: number;
 ...
 */
export async function loadAllLogsFromSessionFile(sessionFile: string, projectPathOverride?: string): Promise<LogOption[]> {
    const { messages, summaries, customTitles, tags, agentNames, agentColors, agentSettings, prNumbers, prUrls, prRepositories, modes, fileHistorySnapshots, contentReplacements, leafUuids, } = await loadTranscriptFile(sessionFile, { keepAllLeaves: true });
    if (messages.size === 0)
        return [];
 ...
 */
export async function getSessionFilesLite(projectDir: string, limit?: number, projectPath?: string): Promise<LogOption[]> {
    const sessionFilesMap = await getSessionFilesWithMtime(projectDir);
 ...
 */
export async function enrichLogs(allLogs: LogOption[], startIndex: number, count: number): Promise<{
    logs: LogOption[];
    nextIndex: number;
}> {
