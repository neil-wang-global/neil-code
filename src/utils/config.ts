...
// Re-entrancy guard: prevents getConfig → getGlobalConfig → getConfig
// infinite recursion when the config file is corrupted. Feature-flag checks
// read GrowthBook settings from global config, which calls getConfig again.
...
export type GlobalConfig = {
    ...
    env: {
        [key: string]: string;
    }; // Environment variables to set for the CLI
 ...
 * @internal
 */
function removeProjectHistory(projects: Record<string, ProjectConfig> | undefined): Record<string, ProjectConfig> | undefined {
    if (!projects) {
        return projects;
    }
...
function saveConfig<A extends object>(file: string, config: A, defaultConfig: A): void {
    ...
    const filteredConfig = pickBy(config, (value, key) => jsonStringify(value) !== jsonStringify(defaultConfig[key as keyof A]));
 ...
 */
function saveConfigWithLock<A extends object>(file: string, createDefault: () => A, mergeFn: (current: A) => A): boolean {
    ...
    try {
        ...
        // Filter out any values that match the defaults
        const filteredConfig = pickBy(mergedConfig, (value, key) => jsonStringify(value) !== jsonStringify(defaultConfig[key as keyof A]));
...
function getConfig<A>(file: string, createDefault: () => A, throwOnInvalid?: boolean): A {
    ...
    try {
        ...
        if (error instanceof ConfigParseError) {
            ...
            // Guard: getGlobalConfig → getConfig can recurse when the config file
            // is corrupted, because feature-flag checks read global config again.
            // Only handle the error on the outermost call.
            if (!insideGetConfig) {
