 ...
 */
export async function fileHistoryTrackEdit(updateFileHistoryState: (updater: (prev: FileHistoryState) => FileHistoryState) => void, filePath: string, messageId: UUID): Promise<void> {
 ...
 */
export async function fileHistoryMakeSnapshot(updateFileHistoryState: (updater: (prev: FileHistoryState) => FileHistoryState) => void, messageId: UUID): Promise<void> {
    if (!fileHistoryEnabled()) {
        return undefined;
    }
 ...
 */
export async function fileHistoryRewind(updateFileHistoryState: (updater: (prev: FileHistoryState) => FileHistoryState) => void, messageId: UUID): Promise<void> {
    if (!fileHistoryEnabled()) {
        return;
    }
 ...
 */
export async function checkOriginFileChanged(originalFile: string, backupFileName: string, originalStatsHint?: Stats): Promise<boolean> {
 ...
 */
function compareStatsAndContent<T extends boolean | Promise<boolean>>(originalStats: Stats | null, backupStats: Stats | null, compareContent: () => T): T | boolean {
 ...
 */
export function fileHistoryRestoreStateFromLog(fileHistorySnapshots: FileHistorySnapshot[], onUpdateState: (newState: FileHistoryState) => void): void {
    if (!fileHistoryEnabled()) {
        return;
    }
...
export async function copyFileHistoryForResume(log: LogOption): Promise<void> {
    ...
    try {
        ...
        await Promise.allSettled(fileHistorySnapshots.map(async (snapshot) => {
            const backupEntries = Object.values(snapshot.trackedFileBackups).filter((backup): backup is typeof backup & {
