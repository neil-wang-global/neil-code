...
export function getPersistenceThreshold(toolName: string, declaredMaxResultSizeChars: number): number {
    ...
    const overrides = getFeatureValue_CACHED_MAY_BE_STALE<Record<string, number> | null>(PERSIST_THRESHOLD_OVERRIDE_FLAG, {});
 ...
 */
export async function persistToolResult(content: NonNullable<ToolResultBlockParam['content']>, toolUseId: string): Promise<PersistedToolResult | PersistToolResultError> {
 ...
 */
export async function processToolResultBlock<T>(tool: {
    name: string;
 ...
 */
export async function processPreMappedToolResultBlock(toolResultBlock: ToolResultBlockParam, toolName: string, maxResultSizeChars: number): Promise<ToolResultBlockParam> {
 ...
 */
async function maybePersistLargeToolResult(toolResultBlock: ToolResultBlockParam, toolName: string, persistenceThreshold?: number): Promise<ToolResultBlockParam> {
 ...
 */
export function provisionContentReplacementState(initialMessages?: Message[], initialContentReplacements?: ContentReplacementRecord[]): ContentReplacementState | undefined {
...
type CandidatePartition = {
    mustReapply: Array<ToolResultCandidate & {
        replacement: string;
    }>;
...
function partitionByPriorDecision(candidates: ToolResultCandidate[], state: ContentReplacementState): CandidatePartition {
    return candidates.reduce<CandidatePartition>((acc, c) => {
...
async function buildReplacement(candidate: ToolResultCandidate): Promise<{
    content: string;
    originalSize: number;
} | null> {
 ...
 */
export async function enforceToolResultBudget(messages: Message[], state: ContentReplacementState, skipToolNames: ReadonlySet<string> = new Set()): Promise<{
    messages: Message[];
    ...
    const shouldSkip = (id: string): boolean => nameByToolUseId !== undefined &&
 ...
 */
export async function applyToolResultBudget(messages: Message[], state: ContentReplacementState | undefined, writeToTranscript?: (records: ToolResultReplacementRecord[]) => void, skipToolNames?: ReadonlySet<string>): Promise<Message[]> {
    if (!state)
        return messages;
 ...
 */
export function reconstructContentReplacementState(messages: Message[], records: ContentReplacementRecord[], inheritedReplacements?: ReadonlyMap<string, string>): ContentReplacementState {
 ...
 */
export function reconstructForSubagentResume(parentState: ContentReplacementState | undefined, resumedMessages: Message[], sidechainRecords: ContentReplacementRecord[]): ContentReplacementState | undefined {
    if (!parentState)
        return undefined;
