...
async function tryWithVersionLock(versionFilePath: string, callback: () => void | Promise<void>, retries = 0): Promise<boolean> {
    const dirs = getBaseDirectories();
...
async function updateLatest(channelOrVersion: string, forceReinstall: boolean = false): Promise<{
    success: boolean;
...
async function installLatestImpl(channelOrVersion: string, forceReinstall: boolean = false): Promise<InstallLatestResult> {
...
async function manualRemoveNpmPackage(packageName: string): Promise<{
    success: boolean;
    error?: string;
    warning?: string;
}> {
...
async function attemptNpmUninstall(packageName: string): Promise<{
    success: boolean;
    error?: string;
    warning?: string;
}> {
