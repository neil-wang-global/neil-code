 ...
 */
export async function captureMemoryDiagnostics(trigger: 'manual' | 'auto-1.5GB', dumpNumber = 0): Promise<MemoryDiagnostics> {
    ...
    const activeRequests = (process as unknown as {
 ...
 */
export async function performHeapDump(trigger: 'manual' | 'auto-1.5GB' = 'manual', dumpNumber = 0): Promise<HeapDumpResult> {
    try {
