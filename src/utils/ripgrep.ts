...
const MAX_BUFFER_SIZE = 20000000; // 20MB; large monorepos can have 200k+ files
/**
...
function ripGrepRaw(args: string[], target: string, abortSignal: AbortSignal, callback: (error: ExecFileException | null, stdout: string, stderr: string) => void, singleThread = false): ChildProcess {
 ...
 */
export async function ripGrepStream(args: string[], target: string, abortSignal: AbortSignal, onLines: (lines: string[]) => void): Promise<void> {
    await codesignRipgrepIfNecessary();
    const { rgPath, rgArgs, argv0 } = ripgrepCommand();
 ...
 */
export const countFilesRoundedRg = memoize(async (dirPath: string, abortSignal: AbortSignal, ignorePatterns: string[] = []): Promise<number | undefined> => {
