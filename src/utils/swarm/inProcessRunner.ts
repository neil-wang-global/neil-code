 ...
 */
function createInProcessCanUseTool(identity: TeammateIdentity, abortController: AbortController, onPermissionWaitMs?: (waitMs: number) => void): CanUseToolFn {
    return async (tool, input, toolUseContext, assistantMessage, toolUseID, forceDecision) => {
            ...
            return new Promise<PermissionDecision>(resolve => {
                ...
                setToolUseConfirmQueue(queue => [
                    ...queue,
                    {
                        ...
                        async onAllow(updatedInput: Record<string, unknown>, permissionUpdates: PermissionUpdate[], feedback?: string, contentBlocks?: ContentBlockParam[]) {
                            if (decisionMade)
                                return;
                            decisionMade = true;
                            abortController.signal.removeEventListener('abort', onAbortListener);
        ...
        return new Promise<PermissionDecision>(resolve => {
            ...
            registerPermissionCallback({
                ...
                toolUseId: toolUseID,
                onAllow(updatedInput: Record<string, unknown> | undefined, permissionUpdates: PermissionUpdate[], _feedback?: string, contentBlocks?: ContentBlockParam[]) {
                    cleanup();
                    persistPermissionUpdates(permissionUpdates);
 ...
 */
function updateTaskState(taskId: string, updater: (task: InProcessTeammateTaskState) => InProcessTeammateTaskState, setAppState: SetAppStateFn): void {
    setAppState(prev => {
 ...
 */
async function sendMessageToLeader(from: string, text: string, color: string | undefined, teamName: string): Promise<void> {
 ...
 */
async function sendIdleNotification(agentName: string, agentColor: string | undefined, teamName: string, options?: {
 ...
 */
async function waitForNextPromptOrShutdown(identity: TeammateIdentity, abortController: AbortController, taskId: string, getAppState: () => AppState, setAppState: SetAppStateFn, taskListId: string): Promise<WaitResult> {
