 ...
 */
function getSessionFileTypeFromInput(toolName: string, toolInput: unknown): 'session_memory' | 'session_transcript' | null {
    switch (toolName) {
        case FILE_READ_TOOL_NAME: {
            const parsed = FileReadTool.inputSchema.safeParse(toolInput);
            if (!parsed.success)
 ...
 */
async function handleSessionFileAccess(input: HookInput, _toolUseID: string | null, _signal: AbortSignal | undefined): Promise<HookJSONOutput> {
